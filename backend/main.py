...
async def ensure_db_schema():
    """
    Auto-migrate schema changes that Tortoise ORM generate_schemas=True might miss
    ...
    try:
        ...
        recipe_updates = [
            ...
            "ALTER TABLE recipes ADD COLUMN views_count INT DEFAULT 0;",
        ...
        restaurant_updates = [
            ...
            "ALTER TABLE restaurants ADD COLUMN views_count INT DEFAULT 0;",
            "ALTER TABLE restaurants ADD COLUMN category VARCHAR(50) NULL;",
        ]

        # Combine all updates
        all_updates = recipe_updates + restaurant_updates
        
        print("Checking for DB schema updates...")
        for sql in all_updates:
            try:
                await conn.execute_script(sql)
                print(f"Executed schema update: {sql}")
            except Exception as e:
                # Ignore "Duplicate column name" (MySQL 1060) or similar errors
                # This means the column already exists, which is fine
                if "Duplicate column name" in str(e) or "1060" in str(e):
                    pass
                else:
                    print(f"Schema update skipped/failed (likely exists): {e}")

        # 3. Create missing tables if needed (Tortoise usually handles this, but for safety)
        # what_to_eat_presets
        try:
            ...
            """)
             ...
             print(f"Table creation check skipped: {e}")
