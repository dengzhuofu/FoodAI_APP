...
export default function App() {
  ...
  const navigate = (page: PageType | string, data?: any) => {
    const targetPage = (page === 'recommend' ? 'home' : page) as PageType;
    
    // 更新导航历史
    setNavigationHistory([...navigationHistory, targetPage]);
  ...
  const renderPage = () => {
    ...
    switch (activePage) {
      ...
      case 'recipe-detail':
        return <RecipeDetailPage navigate={navigate} recipe={pageData} />;
      case 'restaurant-detail':
        return <RestaurantDetailPage navigate={navigate} restaurant={pageData} />;
      default:
        return <RecommendScreen onNavigate={navigate} />;
    }
  };

  const isBottomNavVisible = !showPublish && !['what-to-eat', 'recipe-detail', 'restaurant-detail', 'my-kitchen'].includes(activePage);
            ...
            <div className="flex items-center justify-around h-16">
              ...
              <button
                ...
                className={`flex flex-col items-center gap-1 px-3 py-2 transition-all ${
                  activePage === 'ai-kitchen' ? 'text-orange-500' : 'text-gray-500'
                }`}
              >
                <div className={`transition-transform ${activePage === 'ai-kitchen' ? 'scale-110' : ''}`}>
                  <span className="text-lg">🤖</span>
                </div>
                <span className="text-xs">AI厨房</span>
              </button>
